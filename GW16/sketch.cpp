#include <DallasTemperature.h>// termo sensor library v
#include <OneWire.h>// termo sensor library v
#define ONE_WIRE_BUS 4 //  both temperature sensors are connected to the same 5 pin v

OneWire oneWire(ONE_WIRE_BUS); //v
DallasTemperature sensors(&oneWire); //v
DeviceAddress Sensor1 ={0x28, 0xFF, 0x46, 0x6A, 0xC4, 0x17, 0x04, 0x98};// address for control sensor //v
DeviceAddress Sensor2 = {0x28, 0xAA, 0x51, 0x6E, 0x18, 0x13, 0x2, 0x3A}; // address for control sensor //v
...
void setup(){
  ...
  TenSecCycleCount = 60*TenMinCount + T_EEPROM -1;
